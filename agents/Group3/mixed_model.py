...
class AzaleaGraveNN(AgentBase):
    """
    Baseline agent that uses the original Azalea weights only.
    """
    ...
    def __init__(self, colour: Colour,
                 ...
                 sims=2000,
                 c_puct=1.2):
        # Mark as learning agent so Game.run_with_trajectory can log its moves
        self.is_learning_agent = True

        super().__init__(colour)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        print(f"[NumbaGraveNN] Loading Azalea network from {load_path}")
...
class GraveNNSelfPlay(AzaleaGraveNN):
    """
    Self play only agent.
    Same architecture and search logic as NumbaGraveNN,
    but loads the self play trained weights.
    """

    def __init__(self, colour: Colour,
                 load_path="agents/Group3/models/selfplay_iter_10.pth",
                 sims=2000,
                 c_puct=1.2):
        super().__init__(colour, load_path=load_path, sims=sims, c_puct=c_puct)
        print(f"[GraveNNSelfPlay] Using self play weights from {load_path}")

class AzaleaGraveNNEnsemble(AgentBase):
    """
    Ensemble agent that combines Azalea and self play nets.
    At each evaluation it calls both networks and averages
    their logits and value.
    """

    def __init__(self, colour: Colour,
                 azalea_path="agents/Group3/models/hex11-20180712-3362.policy.pth",
                 selfplay_path="agents/Group3/models/selfplay_iter_10.pth",
                 sims=1500,
                 c_puct=1.2):
        ...
        print(f"[NumbaGraveNNEnsemble] Loading Azalea from {azalea_path}")
        self.net_azalea = load_hex11_pretrained(azalea_path, self.device)
        self.net_azalea.eval()

        print(f"[NumbaGraveNNEnsemble] Loading self play from {selfplay_path}")
        self.net_self = load_hex11_pretrained(selfplay_path, self.device)
        self.net_self.eval()
    ...
    def opening_move(self, turn, board: Board, opponent_move: Move | None):
        # reuse exactly the same opening logic as NumbaGraveNN
        N = board.size
        centre = (N // 2, N // 2)
        near = {
            (centre[0] - 1, centre[1]),
    ...
    def make_move(self, turn, board: Board, opponent_move: Move | None):
        ...
        for _ in range(self.sims):
            ...
            if is_terminal == 1:
                ...
                canon_board, idx_map = AzaleaGraveNN.canonicalise_board(
                    leaf_board, leaf_player, N
                )
                ...
                # both nets
                logits_a, value_a = self.net_azalea(encoded)
                logits_s, value_s = self.net_self(encoded)

                logits = (logits_a + logits_s) / 2.0
                value_t = (value_a + value_s) / 2.0
                value = float(value_t.item())

                raw_canon = torch.softmax(logits[0], dim=0).cpu().numpy()
        ...
        print(
            "[NumbaGraveNNEnsemble] Move =", action,
            "took", time.time() - t0, "seconds",
            f"(NN calls: {nn_calls}, terminal hits: {terminal_hits})",
            "sims =", self.sims,
        )
